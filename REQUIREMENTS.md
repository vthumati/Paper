...
**v1.8 (2026-07-21)** — simplicity pass (feature packs + navigation de-duplication, startup side):
- **Security + dead-code pass** ✅ — two-agent review of the session's changes. **HIGH fixed:** the teardown FK-closure was tenant-blind — because `portfolio_investments.company_entity_id` and `spvs.portco_entity_id` are *cross-tenant* link FKs, tearing down a company could delete another tenant's fund/SPV records. The closure now excludes those link edges (`_LINK_FKS`) and nulls them (`_nullify_links`) instead, so the other tenant keeps its record; the preview no longer counts foreign rows. **Also:** CSV exports neutralise spreadsheet formula injection (`=`/`+`/`-`/`@` prefixing, with a real vector via linked company names); `PUT /entities/{id}/pack` is now owner/admin-only (it's the commercial tier); the empty-only `DELETE /tenants/{id}` also clears draft incorporations. **Dead code removed** (fallout from this session's deletions): orphaned Workflow API-client types/methods and `deleteTenant` in `api.ts`; `Fund`'s now-always-hidden internal sub-tab row + `hideSubtabs` prop (parent nav drives sub-tabs). 2 tests added (cross-tenant teardown safety, CSV injection); 350 pass.
- **Fund nav grouped by job** ✅ — the flat 8-tab "Fund" group splits into three: **Fund management** (Capital & LPs · LP fundraise · Plan & forecast · Financials · LP updates), **Portfolio** (Deal pipeline · Holdings — the old Portfolio tab renamed), and **Monitoring** (portfolio-company KPIs/benchmarks/alerts). Frontend-only regrouping in `EntityDetail` TAB_DEFS/GROUPS; company navigation unchanged.
- **Fund ↔ startup data flow (less friction)** ✅ — (1) a fund's **"Investors" tab is relabelled "LP updates" and moved under the Fund group** for fund entities (it's the LP-update channel, not startup investor relations). (2) A portfolio holding can now be **linked to the portfolio company's own Paper entity** (`portfolio_investments.company_entity_id`, Alembic `f8a9b0c1d2e3`): the add-investment form offers a picker of companies the GP has access to (`GET /funds/{id}/linkable-companies`); linking syncs the company name and adds a **↗ open-workspace** jump. (3) **One-click "Pull financials"** (`POST /funds/{id}/portfolio/{iid}/pull-financials`) ingests the linked company's latest self-reported revenue / cash / burn (from its Finance module) as a portfolio-KPI period — no separate KPI-request round-trip — flowing straight into the fund's Monitoring, signals and benchmarks. Access is guarded: a fund can only link to / pull from companies whose workspace the GP is a member of. 3 tests (`test_portfolio_link.py`).
- **Fund-first workspace (fund side)** ✅ — a fund entity no longer wears the startup workspace. (1) **Fund dashboard**: `FundDashboard` replaces the cap-table dashboard for `type == fund` — SEBI category + carry/hurdle/fee, committed/drawn/uncalled/distributed, NAV/TVPI/DPI, LP + portfolio-company counts, an overdue-compliance nudge and "Manage the fund" quick links (the `entity_dashboard` fund block was enriched with performance, terms, uncalled/distributed and portfolio count). No more "Shares issued 0 / Grant options / price ESOP grants" prompts for a fund. (2) **Promoted fund tabs**: the six Fund sub-tabs are now primary workspace tabs (Capital & LPs · LP fundraise · Portfolio · Monitoring · Deal pipeline · Plan & forecast · Financials) under a "Fund" group, instead of nesting inside one tab of the company nav (`Fund` gains `hideSubtabs`; new tab scope `fundonly`). (3) **Trimmed nav**: Cap Table and Valuations are now company-scoped, so the Ownership group disappears for funds. (4) **Portfolio split**: the ~10-section Portfolio page splits into **Portfolio** (signals, holdings, Schedule of Investments, independent valuation) and **Monitoring** (KPIs, benchmarks, alerts, KPI scheduling, DDQ). Legacy `?tab=fund[&sub=]` deep links redirect to the promoted tabs. No schema change.
- **Workspace / entity teardown (destructive, guarded)** ✅ — a founder can now delete a workspace or an individual entity and everything that hangs off it. `services/teardown.py` walks the foreign-key graph from a root row (a `legal_entity`, or a `tenant`) and deletes the transitive closure of rows that reference it — so indirectly-owned records (an ESOP grant off a stakeholder, a round commitment, a drawdown notice) go too, with no hard-coded table list. The same closure powers a dry-run **preview** (row counts per area). Endpoints: `GET/POST /entities/{id}/teardown[-preview]`, `GET/POST /tenants/{id}/teardown[-preview]`. **Guards:** owner-only (403 otherwise) and a type-to-confirm — the request body must echo the exact name (400 on mismatch). Frontend `TeardownDialog` shows the preview breakdown and only enables the destructive button once the user types the name; wired into the Workspaces list and the Entities list. The safe empty-only `DELETE /tenants/{id}` stays as a quick primitive. 3 tests (`test_teardown.py`). No schema change.
- **Feature packs (Starter / Growth / Scale)** ✅ — a company workspace now carries a commercial **pack** (`legal_entities.pack`, Alembic `e7f8a9b0c1d2`) that is the single axis deciding which tabs and in-tab feature-parts are visible; the lifecycle **stage** is reduced to driving only the "what to do now" checklist and the suggested next step. `app/stages.py` gains `TAB_PACK` / `FEATURE_PACK` / `PACK_INFO` (cumulative starter ⊂ growth ⊂ scale) and `pack_for_stage`; `stage_guide` returns `pack` / `packs` / `suggested_pack` and now sources `tabs` + `features` from the pack. New `PUT /entities/{id}/pack` (companies only, 400 for funds/SPVs). **Starter** = day-one legal hygiene (dashboard, tasks, cap table, ESOP, team, governance, compliance, registers, documents, Startup India, marketplace); **Growth** adds raising (rounds & SAFEs, valuations, diligence, investors, contracts, finance, advisors, data room); **Scale** adds advanced cap-table actions (waterfall, anti-dilution, demat, corporate actions), managed admin and fund/SPV administration. Frontend: `EntityDetail` gains a Plan selector + a suggested-pack banner beside the Stage picker; the "All features" toggle becomes "Show everything / Plan view". This **trims the day-one surface** and folds the old two-axis (stage-tabs + feature-flags) gating into one. `test_stage.py` / `test_journey.py` updated for the pack model.
- **Navigation de-duplication** ✅ — (1) **one word, "workspace"**, everywhere the tenant layer appears (was Workspaces / Organisations / organisation); (2) **Documents consolidated** — Files and Data Room fold into a single **Documents** tab with **Library** and **Data room** sub-views (data room gated to Growth), and tax-filing records move to **Compliance** ("Tax filings"); (3) **Workflows removed** — the dashboard stage-checklist, incorporation wizard and Rounds & SAFEs hub already drive those multi-step processes (frontend `Workflows.tsx` / `Files.tsx` deleted; command-palette and Getting-Started copy updated; the diligence readiness "data room" link now points at Documents). No new capability; three top-level tabs removed from the company workspace.

**v1.7 (2026-07-20)** — portfolio intelligence (Vestberry competitive gap analysis, fund side):
- **QA / polish pass (Visible + Rundit batches)** ✅ — two-agent correctness review of the recent build-out; the four flagged suspect areas (report-period month arithmetic, follow-on running-total vs delta, expense-sign in `fund_financials`, benchmark quartile math) all verified correct. Fixes: (1) `delete_kpi_definition` now cascade-deletes any `MetricAlertRule` on `custom.<key>` so a deleted custom metric can't leave a silent no-op rule in the catalog; (2) `POST /portal/updates/{id}/view` engagement recording now works for fund LPs and SPV co-investors who see an update via fund/SPV membership (previously 404'd for lack of a company-level `InvestorAccess` row) — new `can_view_update` visibility helper mirrors the three portal surfacing paths; (3) frontend: duplicate React keys on the update-engagement table (keyed on `id`), numeric-input guards on the capital-call / distribution / add-investment buttons, and `window.alert` → toast on report generation. 2 tests added (`test_rundit_functional.py`). No schema change.
- **Rundit functional batch (fund + startup side)** ✅ — (1) **follow-on investment rounds**: new `InvestmentRound` — per-round history (label, instrument, amount, date, note) under each portfolio company; the parent investment's amount stays the running total cost so SOI / NAV / financials are untouched (`GET/POST /funds/{id}/portfolio/{iid}/rounds`; "Investment history" + add-follow-on in the tear-sheet panel); (2) **fund expense ledger**: new `FundExpense` (date, category legal/audit/administration/diligence/other, amount, note) — actual expenses now reduce net-operations and derived cash in the FR-J-14 financial statements, which still tie out ("Expense ledger" section under Financial statements); (3) **team notes on portfolio companies**: new `CompanyNote` — internal comments with author attribution, never shown to LPs or the company ("Team notes" in the tear-sheet panel); (4) **CSV exports**: `GET /funds/{id}/export/holdings|capital-accounts|kpis` (Content-Disposition downloads; Export buttons on SOI, Capital accounts and Monitoring); (5) **investor-update audiences (FR-K-2 extended)**: `investor_updates.audience` optional email list — restricted updates are invisible to (and unviewable by) investors outside the list; audience multi-select in the composer, "→ N investors" chip, engagement recipients honour the audience. Alembic `d6e7f8a9b0c1`. 5 tests (`test_rundit_functional.py`).
- **Rundit visual batch (fund + startup side)** ✅ — from a visual analysis of rundit.com product screenshots: (1) **web-native LP report view** (extends FR-J-22/FR-J-18) — the quarterly report as a magazine-style page (serif hero figures with growth arrows, activity list, per-company blocks with ownership / invested / fair value / MOIC / holding years and a **FV-growth pill**, valuation status) via new `lp_report_data` + `GET /funds/{id}/lp-report/preview` (GP, defaults to the last completed quarter) and LP-scoped `GET /portal/funds/{id}/lp-report`; "Report view" toggle beside the report generator and a "Quarterly report" button on every portal fund card (shared `LpReportView` component; the document export stays); (2) **pinned summary rows** — TOTAL / AVERAGE / MEDIAN rows atop the benchmark matrix (`stats` gains `avg`); (3) **reporting-status roster** — per-company chips in Monitoring (✓ latest period / requested / submitted / overdue / never reported); (4) **update engagement roll-up** — an ALL-UPDATES + per-update table (recipients × opened × opening rate) over the FR-K-2 view tracking. 1 test (preview + portal scoping in `test_lp_report.py`).
